...
class Main(APIView):
    def get(self, request):
        ...
        for feed in feed_object_list:
            author = User.objects.filter(email=feed.email).first()  # 피드 작성자의 정보를 가져옵니다.
            reply_object_list = Reply.objects.filter(feed_id=feed.id)
            reply_list = []
            for reply in reply_object_list:
                reply_author = User.objects.filter(email=reply.email).first()  # 댓글 작성자의 정보를 가져옵니다.
                reply_list.append(dict(reply_content=reply.reply_content, nickname=reply_author.nickname))
